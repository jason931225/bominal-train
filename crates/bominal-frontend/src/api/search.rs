...
pub async fn search_trains(
    ...
    match provider.as_str() {
        "SRT" => search_srt(&departure, &arrival, date.as_deref(), time.as_deref()).await,
        "KTX" => search_ktx(&departure, &arrival, date.as_deref(), time.as_deref()).await,
...
async fn search_srt(
    ...
    let trains = client
        .search_train(departure, arrival, date, time, false)
        .await
        .map_err(|e| ServerFnError::new(format!("SRT search error: {e}")))?;
...
async fn search_ktx(
    ...
    let trains = client
        .search_train(departure, arrival, date, time, false)
        .await
        .map_err(|e| ServerFnError::new(format!("KTX search error: {e}")))?;

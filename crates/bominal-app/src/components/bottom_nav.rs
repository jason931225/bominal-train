...
fn icon_home() -> impl IntoView {
    view! {
        <svg aria-hidden="true" class="lg-bottom-nav__icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.9">
...
pub fn BottomNav() -> impl IntoView {
    ...
    let item_class = move |path: &'static str, exact: bool| {
        if is_active(path, exact) {
            "lg-bottom-nav__link lg-bottom-nav__link--active"
        } else {
            "lg-bottom-nav__link"
        }
    };
    ...
    view! {
        <nav aria-label="Mobile navigation" class="lg-bottom-nav">

...
use super::super::srt::passenger::{PassengerGroup, PassengerType, combine_passengers, total_count};
...
impl KtxClient {
    ...
    async fn fetch_encryption_params(&self) -> Result<(String, String), ProviderError> {
        // Python reference sends only {"code": "app.login.cphd"} — no Device/Version
        let form: Vec<(&str, &str)> = vec![("code", "app.login.cphd")];
        ...
        // The /code response nests key+idx under "app.login.cphd"
        let cphd = parsed.json().get("app.login.cphd");
        ...
        if key.is_empty() {
            return Err(ProviderError::UnexpectedResponse {
                status: 200,
                body: format!(
                    "Missing encryption key from /code endpoint. Response: {}",
    ...
    pub async fn login(&mut self, login_id: &str, password: &str) -> Result<(), ProviderError> {
        ...
        // SID must share the same timestamp as the DynaPath token
        let (builder, sid) =
        ...
        if !parsed.is_success() {
            ...
            // KTX returns two error formats:
            // Standard: {"strResult":"FAIL","h_msg_cd":"...","h_msg_txt":"..."}
            // Version gate: {"code":-1406,"message":"..."}
            let msg = if !parsed.message().is_empty() {
                format!("[{}] {}", parsed.message_code(), parsed.message())
            } else {
                // Fallback to code/message format
                let code = parsed.json().get("code").and_then(|v| v.as_i64()).unwrap_or(0);
                let message = parsed.json().get("message").and_then(|v| v.as_str()).unwrap_or("Unknown error");
                format!("[{}] {}", code, message)
            };
            warn!(provider = "KTX", error_msg = %msg, "KTX login failed");
            return Err(ProviderError::LoginFailed { message: msg });
        }
    ...
    pub async fn logout(&mut self) -> Result<(), ProviderError> {
        ...
        let url = wreq::Url::parse_with_params(
            &Endpoints::url(Endpoints::LOGOUT),
            &form,
        )
        .map_err(|_| ProviderError::UnexpectedResponse {
            status: 0,
            body: "Failed to build logout URL".to_string(),
        })?;
    ...
    pub async fn search_train_with_passengers(
        ...
        // Python sends POST with params= (query string), not form body.
        // Build URL with query params, then re-create the request with dynapath.
        let search_url = wreq::Url::parse_with_params(
            &Endpoints::url(Endpoints::SEARCH),
            &params,
        )
        .map_err(|_| ProviderError::UnexpectedResponse {
            status: 0,
            body: "Failed to build search URL".to_string(),
        })?;
        let (builder, _) =
            self.request_with_dynapath(wreq::Method::POST, search_url.as_str());
    ...
    pub async fn reserve_with_passengers(
        ...
        let reserve_url = wreq::Url::parse_with_params(
            &Endpoints::url(Endpoints::RESERVE),
            &params_ref,
        )
        .map_err(|_| ProviderError::UnexpectedResponse {
            status: 0,
            body: "Failed to build reserve URL".to_string(),
        })?;
        let (builder, _) =
            self.request_with_dynapath(wreq::Method::GET, reserve_url.as_str());
        let resp = builder.send().await?;
    ...
    pub async fn get_reservations(&self) -> Result<Vec<KtxReservation>, ProviderError> {
        ...
        let view_url = wreq::Url::parse_with_params(
            &Endpoints::url(Endpoints::RESERVATION_VIEW),
            &params,
        )
        .map_err(|_| ProviderError::UnexpectedResponse {
            status: 0,
            body: "Failed to build reservation view URL".to_string(),
        })?;
        let resp = self.client.get(view_url).send().await?;
    ...
    pub async fn ticket_info(&self, pnr_no: &str) -> Result<Vec<KtxTicket>, ProviderError> {
        ...
        let list_url = wreq::Url::parse_with_params(
            &Endpoints::url(Endpoints::RESERVATION_LIST),
            &params,
        )
        .map_err(|_| ProviderError::UnexpectedResponse {
            status: 0,
            body: "Failed to build reservation list URL".to_string(),
        })?;

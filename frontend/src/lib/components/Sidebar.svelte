...
<nav
	class="lg-glass-sidebar fixed left-0 top-0 h-full w-[72px] flex flex-col items-center py-6 gap-1 border-r"
	style="z-index: 40;"
>
	<!-- Brand wordmark -->
	<a
		href="/home"
		class="mb-6 flex items-center justify-center w-10 h-10"
		aria-label="Bominal"
	>
		<span
			class="app-brand-wordmark bg-clip-text text-transparent font-bold text-lg tracking-tight select-none"
		>
			B
		</span>
	</a>
	...
	{#each navItems as item}
		...
		<a
			href={item.href}
			class="flex flex-col items-center justify-center w-14 py-3 rounded-2xl transition-all duration-200 group squish"
			class:lg-active={active}
		...
		>
			...
			<span
				class="text-[10px] mt-1 font-medium leading-tight transition-colors duration-200"
				style="color: {active
					? 'var(--color-brand-text)'
					: 'var(--color-text-tertiary)'};"
			>
...
<!-- Spacer so main content isn't hidden behind sidebar -->
<div class="w-[72px] flex-shrink-0" aria-hidden="true"></div>
v>

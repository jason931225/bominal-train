...
Rewrite the bominal-train frontend from SvelteKit 2.16 SPA to Leptos 0.8 SSR with selective hydration (islands architecture). The Leptos app will be served directly by the existing Axum server, eliminating the separate npm build step. All 14 routes, 15 components, 3 stores, 6 API modules, the i18n system (ko/en/ja), SSE real-time updates, and the glass morphism design system will be faithfully ported. No backend API changes required.

**Replaces:** Previous SvelteKit feature-wiring roadmap (2026-03-24)
**Migration scope:** 10 route groups, 15 components, 283 i18n keys x 3 locales, SSE, WebAuthn, Evervault interop
...
- [ ] **Phase 1: Foundation** - Crate setup, cargo-leptos config, workspace deps, dual compilation verified
- [ ] **Phase 2: Core Infrastructure** - i18n, utils, types, server functions (API layer), state management
- [ ] **Phase 3: Shell and Navigation** - Root App with router, layout, Sidebar, BottomNav
...
### Phase 1: Foundation
**Goal**: Leptos 0.8 crate compiles in both SSR and hydrate modes with cargo-leptos orchestrating the build
**Depends on**: Nothing (first phase)
**Requirements**: FND-01, FND-02, FND-03, FND-04, FND-05
**Success Criteria** (what must be TRUE):
  1. `bominal-frontend` dead crate deleted, workspace clean
  ...
  4. `Cargo-leptos.toml` exists with correct bin-package/lib-package/feature config
  5. `cargo check --features ssr` succeeds on bominal-app
  6. `cargo check --target wasm32-unknown-unknown --features hydrate` succeeds on bominal-app
**Plans:** 2 plans
Plans:
- [ ] 01-01-PLAN.md -- Delete dead crate, rewrite bominal-app Cargo.toml with SSR/hydrate features, add workspace deps
- [ ] 01-02-PLAN.md -- Configure cargo-leptos build system, create minimal App shell, verify dual compilation
...
**Goal**: Tailwind CSS builds without npm, Dockerfile updated, dev workflow uses cargo-leptos
**Depends on**: Phase 8
**Requirements**: CSS-01, CSS-02, BUILD-01, BUILD-02
**Success Criteria** (what must be TRUE):
  1. Glass morphism design system renders identically to SvelteKit version
...
| Delete bominal-frontend | Dead code with no source files, misleading Cargo.toml |

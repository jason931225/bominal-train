...
if ! matches_pattern '^\s*web-e2e:\s*$' "$COMPOSE_FILE"; then
  echo "FAIL: infra/docker-compose.yml missing web-e2e service" >&2
  exit 1
fi

if ! matches_pattern '^\s*profiles:\s*\["e2e"\]\s*$' "$COMPOSE_FILE"; then
  echo "FAIL: web-e2e service must be gated behind profile e2e" >&2
  exit 1
fi

if ! matches_pattern '^\s*dockerfile:\s*Dockerfile\.e2e\s*$' "$COMPOSE_FILE"; then

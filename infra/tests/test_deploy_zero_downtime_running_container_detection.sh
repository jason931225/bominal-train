...
if ! rg -q "up -d --wait postgres redis api worker worker-restaurant web caddy" "$FIRST_CALLS"; then
...
if rg -q "up -d --wait postgres redis api worker worker-restaurant web caddy" "$UPDATE_CALLS"; then

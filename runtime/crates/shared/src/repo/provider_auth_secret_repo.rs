...
pub const PROVIDER_AUTH_SECRET_UPDATE_METADATA_SQL: &str = "update provider_auth_secrets set redacted_metadata = cast($4 as jsonb), updated_at = $5 where provider = $1 and subject_ref = $2 and credential_kind = $3 and revoked_at is null";
...
pub fn update_provider_auth_secret_metadata_query<'q>(
    provider: &'q str,
    subject_ref: &'q str,
    credential_kind: &'q str,
    redacted_metadata: &'q Value,
    updated_at: DateTime<Utc>,
) -> Query<'q, Postgres, PgArguments> {
    sqlx::query(PROVIDER_AUTH_SECRET_UPDATE_METADATA_SQL)
        .bind(provider)
        .bind(subject_ref)
        .bind(credential_kind)

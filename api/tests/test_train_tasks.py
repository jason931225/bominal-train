...
@pytest.mark.asyncio
async def test_srt_wait_reserve_path_uses_special_for_special_preferred(
    db_session_factory,
    db_session,
    monkeypatch,
):
    ...
    class FakeSRTProvider:
        ...
        async def search(self, **kwargs):
            ...
            schedule = ProviderSchedule(
                ...
                train_no="S310",
                availability={"general": False, "special": False},
                metadata={"reserve_wait_code": "3"},
            )
            return ProviderOutcome(ok=True, data={"schedules": [schedule]})

        async def reserve(self, **kwargs):
            self.reserve_calls.append(kwargs["seat_class"])
            return ProviderOutcome(ok=True, data={"reservation_id": "srt-rsv-waitlist"})
    ...
    user = User(
        email="srt-waitlist-worker@example.com",
        password_hash="x",
        display_name="SRT Waitlist User",
        role_id=2,
    )
    db_session.add(user)
    await db_session.flush()
    ...
    task = Task(
        ...
        spec_json={
            ...
            "arr": "부산",
            "date": utc_now_naive().date().isoformat(),
            "selected_trains_ranked": [{"schedule_id": "srt-waitlist-only", "departure_at": deadline.isoformat(), "rank": 1}],
            "passengers": {"adults": 1, "children": 0},
            "seat_class": "special_preferred",
            "auto_pay": False,
        },
        idempotency_key="srt-waitlist-worker",
    )
    db_session.add(task)
    await db_session.flush()
    ...
    assert fake_provider.reserve_calls == ["special"]
